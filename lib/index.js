...
import emojiCountries from './resources/countries-emojis.json'

const showEmoji = Platform.OS === 'ios';
...
export default class PhoneInput extends Component {
    ...
    getEmoji(iso2) {
        if (iso2) {
            let key = iso2.toUpperCase();
            return emojiCountries[key].flag;
        } else {
            return '';
        }
    }
        ...
        return (
            <View style={[styles.container, this.props.style]}>
                <TouchableWithoutFeedback onPress={this.onPressFlag}>
                    { showEmoji ? <Emoji name={this.getEmoji(this.state.iso2)}
                                         onPress={this.onPressFlag}
                                         style={this.props.emojiStyle} /> : <Image source={Flags.get(this.state.iso2)}
